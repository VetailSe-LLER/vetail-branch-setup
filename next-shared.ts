...
async function matchFederatedPage(path: string) {
...
export function createFederatedCatchAll(remotes: string[]) {
  ...
  FederatedCatchAll.getInitialProps = async (ctx) => {
    ...
    try {
      ...
      const FederatedPage = await loadRemote(remote + mod).then(
